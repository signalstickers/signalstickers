...
export function Provider(props: React.PropsWithChildren<Record<string, unknown>>) {
  ...
  const [previousSortOrder, setPreviousSortOrder] = React.useState<SortOrder | undefined>(sortOrder);
  ...
  React.useEffect(() => {
    ...
    // restore sort order to its last known value.
    if (!searchQuery && sortOrder === 'relevance' && previousSortOrder) {
      setSortOrder(previousSortOrder);
      return;
    }

    // If the previous search query was empty and the new search query is
    // non-empty and the previous sort order was not "relevance", set the sort
    // order to "relevance".
